...
import { action as ordersCreate } from "./webhooks.orders.create";
...
// /app/webhooks/... — which matched no route, and is what returned 404 to
// Shopify's compliance probe.
//
// The right fix is an application_url with no path, which the config files now
// enforce. This exists so a stale or hand-typed registration can't take the
// compliance webhooks down again: either path reaches the same handler, and the
// handler verifies the HMAC exactly as before. It grants no access — an
// unsigned request still gets 401 here.
//
// The trailing underscore in the filename (`app_.`) keeps this out of the
// app.tsx layout. Nested under it, every delivery would hit
// authenticate.admin() and be bounced to OAuth instead of being processed.
const HANDLERS: Record<string, (args: ActionFunctionArgs) => Promise<Response>> = {
